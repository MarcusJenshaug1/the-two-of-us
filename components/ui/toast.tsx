...
export function ToastProvider({ children }: { children: ReactNode }) {
    ...
    const icon = (type: ToastType) => {
        switch (type) {
            ...
            case 'love': return <Heart className="w-4 h-4 text-rose-600 shrink-0" />
            case 'info': return <Info className="w-4 h-4 text-muted-foreground/60 shrink-0" />
        }
    }
            ...
            <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center gap-2 pointer-events-none w-full max-w-sm px-4">
                {toasts.map(t => (
                    <div
                        key={t.id}
                        className={clsx(
                            'pointer-events-auto w-full flex items-center gap-3 px-4 py-3 rounded-xl border shadow-lg backdrop-blur-md bg-card text-card-foreground animate-in fade-in slide-in-from-top-2 duration-300',
                            t.type === 'success' && 'border-success/30',
                            t.type === 'error' && 'border-destructive/30',
                            t.type === 'love' && 'border-rose-300',
